...
# Added MLE, MAP (Ridge), MAP (Lasso), and Elastic Net specifically
method = st.sidebar.selectbox("Regression Strategy", 
                            ["MLE (OLS)", "MAP (Ridge)", "MAP (Lasso)", "Elastic Net"])

# Lambda Control (10^x for smooth movement)
exp = st.sidebar.slider("Regularization Strength (λ)", -2.0, 4.0, 0.0, 0.1)
lam = 10**exp

# Elastic Net Alpha (Mixing Ratio) - Only shows when Elastic Net is selected
l1_ratio = 0.5
...
if jitter_active:
    # Adds random noise to see if the model's weights stay stable
...
with tab1:
    ...
    with col1:
        ...
        fig_path.update_layout(xaxis_type="log", template="plotly_white", height=400,
                              xaxis_title="Lambda (Strength)", yaxis_title="Weight Magnitude")
...
with tab2:
    ...
    4. **Why Lasso is 'Sparse'**: Notice that Lasso creates a **pointy, diamond-shaped bottom**. This 'pointiness' is what makes weights hit exactly zero.
    """)

    # 3D Plot Logic
    w_grid = np.linspace(-2, 2, 50)
    W1, W2 = np.meshgrid(w_grid, w_grid)
    # The 'Target' (where the model wants to go without penalty)
    RSS = (W1 - 1)**2 + (W2 - 1)**2 
    ...
    fig_3d.update_layout(
        title=f"3D Surface for {method}",
        scene=dict(xaxis_title='Weight A', yaxis_title='Weight B', zaxis_title='Total Loss'),
        height=700, margin=dict(l=0, r=0, b=0, t=40)
    )
    st.plotly_chart(fig_3d, use_container_width=True)

with tab3:
    st.subheader("Insurance Dataset Summary")
